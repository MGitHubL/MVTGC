...
class MVTGCDataSet(Dataset):
    def __init__(self, args, file_path, neg_size, hist_len, directed, transform=None):
        self.neg_size = neg_size
        self.hist_len = hist_len
        self.directed = directed
        self.transform = transform
        self.View_RW_path = args.View_RW_path
        self.View_PE_path = args.View_PE_path
        self.View_A_path = args.View_A_path
    ...
    def get_View_RW(self):
        node_emb = dict()
        with open(self.View_RW_path, 'r') as reader:
            reader.readline()
            for line in reader:
    ...
    def get_View_PE(self):
        node_emb = dict()
        with open(self.View_PE_path, 'r') as reader:
            reader.readline()
            for line in reader:
                embeds = np.fromstring(line.strip(), dtype=float, sep=' ')
                node_id = embeds[0]
    ...
    def get_View_A(self):
        node_emb = dict()
        with open(self.View_A_path, 'r') as reader:
            reader.readline()
            for line in reader:
                embeds = np.fromstring(line.strip(), dtype=float, sep=' ')
                node_id = embeds[0]
